...
import { WaterTableColumn } from './components/Table/column-type';
...
const WaterPage = (props: Props) => {
  ...
  const [formModal, setFormModal] = useState<Water>();

  // handle loading data
  const [loading, setLoading] = useState<boolean>(true);
  const [water, setWater] = useState<Water[] | undefined>([]);
  const { data: session } = useSession();
  const siteId = session?.user.userInfo.siteId;
  const http = UseAxiosAuth();
  const siteName = session?.user.userInfo.siteName;
  const getListWaterApi = async (http: AxiosInstance, siteId: string | undefined) => {
    try {
      const responseData = await fetchListWaterData(siteId, http);
      setWater(responseData.data as Water[]);
      setLoading(false);
    } catch (error) {
      console.error('Error calling API get list water:', error);
    }
  };
  const {
  ...
  const checkRowSelection = {
    onChange: (selectedRowKeys: React.Key[], selectedRows: Water[]) => {
  ...
  const onChange: TableProps<Water>['onChange'] = (
    pagination,
    filters,
    sorter,
    extra
  ...
  const handleDetails = async (record: Water) => {
    setFormModal(record);
    setUpdateState(true);
  };
            ...
            <Modal
              title={
                <div>
                  ...
                  <span>Do you want to delete this water source?</span>
                </div>
              }
              open={deleteState}
              onOk={deleteMultiple}
            ...
            >
              <Content style={{ padding: '0 24px', minHeight: 280 }}>
                <Table
                  ...
                  columns={WaterTableColumn()}
                  dataSource={water?.map(waters => ({
                    ...waters,
                    // onDetails: () => handleDetails(fertilizer.id!),
                    onDelete: () => handleDelete(waters.id!)
