...
import { WaterPayLoadRequest } from '@/services/Admin/Water/Payload/request/addWaterPayLoad';
import { updateWaterApi } from '@/services/Admin/Water/updateWaterService';
...
const UpdateLandModal = ({
  ...
  const handleUpdate = async (values: WaterPayLoadRequest) => {
    const payLoadAddPos: Position[] = [
      { lat: events?.onDragEnd?.lat, long: events?.onDragEnd?.lng }
    ];

    const allValues: WaterPayLoadRequest = {
      ...values,
      positions: payLoadAddPos
    };

    const res = await updateWaterApi(http, allValues, params.dataRow?.id);
            ...
            >
              ...
              >
                <Form.Item
                  name='name'
                  label={
                    <TitleLabelFormItem name='Water source Name'></TitleLabelFormItem>
                  }
                >
                  <Input size='large' />
                </Form.Item>
                ...
                <Form.Item
                  name='notes'
                  label={<TitleLabelFormItem name='Notes'></TitleLabelFormItem>}
                >
                  <Input size='large' />
                </Form.Item>
              </Form>
