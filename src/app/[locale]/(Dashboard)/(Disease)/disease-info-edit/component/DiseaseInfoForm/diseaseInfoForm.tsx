        ...
        <>
            {diseaseInfoDeatail && (
                <Form
                    layout="vertical"
                >
                    <Form.Item initialValue={diseaseInfoDeatail.diseaseName} label={t('disease_name')} name="diseaseName" rules={[{ required: true, message: t('msg_disease_name_require') }]}>
                        <Input onChange={handleOnchangeDiseaseName} disabled={loadings}/>
                    </Form.Item>
            ...
            {displayModalUpdate && msgUpdate && (
                <ModalComponent title={t('edit_disease_info')} body={msgUpdate} open={displayModalUpdate} handleClose={handleClose}/>
            )}
        </>
    );
}
